...
// #ifndef WITH_POLICY_CHECK
	...
	db.msg_count=0;
	db.corrective_msg_count=0;
	db.msg_to_ignore=db.config->hot_start_ignore;
	db.pengine = new_policy_engine();
	db.inv_engine = new_invariant_engine();
	...
	log__printf(NULL,MOSQ_LOG_INFO,"Policies added:%s",db.config->policy_file);
	...
	log__printf(NULL,MOSQ_LOG_INFO,"Propositons added:%s",db.config->prop_file);
