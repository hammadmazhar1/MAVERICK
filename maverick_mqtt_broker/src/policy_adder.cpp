...
bool policy_engine::monitor(sys_state st)
{
    ...
    {
        if(!policy_evaluators[i]->policy_check(policy_asts[i], &st)){
            std::cout<<"Policy violated:" << i <<std::endl;
             return false;  
        }
    }
...
bool invariant_engine::add_invariant(string invariant,string actions)
{
    ...
    for (size_t i = 0; i != action_strings.size();++i){
        // std::cout << action_strings[i] << " " <<action_strings[i].compare("(none,none)") <<std::endl;
        if ((int)action_strings[i][0] !=13 && action_strings[i].compare("(none,none)")!=0 ){
            pair<string,void*> action = parse_action(action_strings[i]);
            parsed_actions.push_back(action);
        }
    }
    // std::cout <<parsed_actions.size() <<std::endl;
...
std::pair<string,void*> invariant_engine::parse_action(string action_string){
    ...
    if (action_topic.find("int")!= string::npos){
        ...
        *val = atoi(action_value.c_str());
        ...
        if (strcmp(action_value.c_str(),"true")==0){
            *val = true;
        } else{
            *val = false;
        }
        value = (void*) val;
        // std::cout <<"Variable: " << action_topic << " Value:"<< *(bool*) value << " Token: " <<action_value <<std::endl;

    } else if (action_topic.find("str") != string::npos){
    
        // std::cout<<"Constant str: "<< action_value <<std::endl;
        char* val{new char[action_value.length()+1]};
