...
int handle__publish(struct mosquitto *context)
{
	...
	db.msg_count++;
	log__printf(NULL,MOSQ_LOG_INFO,"%d messages received",db.msg_count);
	struct rusage mem_usage;
	int mem_res = getrusage(RUSAGE_SELF,&mem_usage);
	log__printf(NULL,MOSQ_LOG_INFO,"Maximum memory usage %ld",mem_usage.ru_maxrss);
	std::string msg_topic = std::string(stored->topic);
	if (stored->payloadlen >0 and db.msg_count>db.msg_to_ignore){
		if (msg_topic.find("/set") != std::string::npos || msg_topic.find("/cmd") != std::string::npos ){
			auto pol_eval_start = steady_clock::now();
			if(policy_engine_monitor(stored)){
				...
				auto pol_eval_finish = steady_clock::now();
				double pol_eval_time = duration_cast<duration<double>> (pol_eval_finish-pol_eval_start).count();
				log__printf(NULL, MOSQ_LOG_INFO, "Time to Evaluate Policy: %.3f",pol_eval_time);
			} else{
			...
			if (corrective_actions.size()>0){
				...
				log__printf(NULL, MOSQ_LOG_INFO, "Time to Evaluate Invariants: %.3f",pol_eval_time);
				uint16_t source_mid = stored->source_mid;
				uint16_t mid = stored->mid;
				for(std::size_t i = 0; i <corrective_actions.size();++i){
					// std::cout << "queueing corrective message" <<std::endl;
					source_mid++;
					mid++;
					...
					if (action.first.find("int") != std::string::npos){
						...
						// std::cout<<"String command: "<<action.first << std::endl;
						// std::cout<<"String data: "<<data_str << std::endl;
						corrective_msg->payload = action.second;
						corrective_msg->payloadlen = data_str.length();
					}
					// std::cout<<"Sending Command" << (char*) corrective_msg->payload << " To " << corrective_msg->topic <<std::endl;
					// sub__messages_queue(context->id, stored->topic, stored->qos, stored->retain, &stored)
					switch(corrective_msg->qos){
						case 0:
							log__printf(NULL, MOSQ_LOG_INFO, "Forwarding corrective PUBLISH with QoS 0 ");
						...
						case 1:
							if(corrective_msg->qos == 1
								...
								// std::cout << 'f' << std::endl;
								db__msg_store_ref_dec(&corrective_msg);
							}
							...
							rc2 = sub__messages_queue(context->id, corrective_msg->topic, corrective_msg->qos, corrective_msg->retain, &corrective_msg);
							log__printf(NULL, MOSQ_LOG_INFO, "Forwarding corrective PUBLISH with QoS 1 ");
							...
							break;
						case 2:
							log__printf(NULL, MOSQ_LOG_INFO, "Forwarding corrective PUBLISH with QoS 2 ");
							if(dup == 0){
								res = db__message_insert(context, corrective_msg->source_mid, mosq_md_in, corrective_msg->qos, corrective_msg->retain, corrective_msg, NULL, false);
							}else{
								res = 0;
							}
							...
							* due to queue. This isn't an error so don't disconnect them. */
							...
							if(!res){
								if(send__pubrec(context, corrective_msg->source_mid, 0, NULL)) rc = 1;
							}else if(res == 1){
								rc = 1;
							}
							...
							break;
					db__msg_store_ref_dec(&corrective_msg);
					db.corrective_msg_count++;
					log__printf(NULL,MOSQ_LOG_INFO,"%d corrective messages sent",db.corrective_msg_count);
		...
		update_system_state(stored);
		log__printf(NULL,MOSQ_LOG_INFO,"Ignoring msg for hot start");
	}
	}
