...
bool update_system_state(struct mosquitto_msg_store* msg){
	...
	if (std::string(msg_topic).find("int") != std::string::npos){
		int* x = new int();
		if (std::string((char*) msg_value).find("NULL")!= std::string::npos){
			*x = 0;
		} else{
		...
		bool* x = new bool();
		if (strcmp((char*)msg_value,"ON") ==0 || strcmp((char*)msg_value,"CLOSED") ==0 || strcmp((char*)msg_value,"true") ==0){
			*x = true;
		} else{
			*x = false;
		}
...
bool policy_engine_monitor(struct mosquitto_msg_store* msg){
	...
	if (std::string(msg_topic).find("int") != std::string::npos){
		...
		bool* x = new bool();
		if (strcmp((char*)msg_value,"ON") ==0 || strcmp((char*)msg_value,"CLOSED") ==0 || strcmp((char*)msg_value,"on") ==0 || strcmp((char*)msg_value,"true") ==0 || strcmp((char*)msg_value,"active") ==0){
			*x = true;
		} else{
			*x = false;
		}
